...
**Answer**: The frontend currently treats the tags string as an array of characters instead of splitting it by commas. This causes each character to display as a separate tag. The issue originates from the Angular component responsible for creating articles:
...
**Answer**: The backend does not properly save or return new tags to the frontend, so the Popular Tags section does not update after creating new tags. The issue originates from the NestJS service responsible for managing articles:
